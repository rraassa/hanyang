...
export default function MainCards({ onNavigate }) {
  ...
  const handleNavigate = () => {
    if (typeof onNavigate === "function") {
      onNavigate(); // App.js에서 전달된 상태변경 함수 실행
    } else {
  ...
  return (
    <section className="flex justify-center items-start py-20 -mt-20">
      <div className="relative flex rounded-2xl rounded-tr-[4rem] bg-[#fafaf5] shadow-[0_10px_30px_rgba(0,0,0,0.2)] px-12 py-12 pb-6 w-[900px] overflow-visible z-0">
        {/* 파란 카드 - 양도 */}
        <div
          className="bg-[#0E2A7B] text-white w-72 h-72 rounded-2xl p-4 flex flex-col justify-between absolute -top-10 -left-6 shadow-xl z-10 transition-transform duration-200 hover:scale-105 cursor-pointer"
        ...
        {/* 보라 카드 - 양수 */}
        <div className="bg-[#4C2E91] text-white w-72 h-72 rounded-tl-2xl rounded-bl-2xl rounded-br-2xl p-4 rounded-tr-[4.5rem] flex flex-col justify-between absolute -top-10 left-[18rem] shadow-xl z-20 transition-transform duration-200 hover:scale-105">
          <div className="text-left mt-5 mb-5 pl-3">
